...
import { CompositeOptions, WebColors, TextOptions, Size, rgbaColor, Composite, IText, EntityType, Game, Shape, Rect, Label } from '../m2c2kit';
  ...
  export class Button extends Composite implements IText {
    ...
    override initialize(): void {
      ...
      const buttonRectangle = new Shape({
        rect: new Rect({ size: this.size}), cornerRadius: this.cornerRadius,
        fillColor: this._backgroundColor
      })
      this.addChild(buttonRectangle);
    
      const buttonLabel = new Label({ text: this.text, fontSize: this.fontSize, fontColor: this.fontColor })
