...
import { Composite, CompositeOptions, Entity, EntityType, Point, Rect, rgbaColor, Shape, Size, WebColors } from "../m2c2kit";
...
interface GridChild {
  ...
  export class Grid extends Composite {
    ...
    gridBackgroundColor: rgbaColor = [0, 0, 233, .25];
    ...
    override initialize(): void {
      ...
      for (let col = 1; col < this.columns; col++) {
        const verticalLine = new Shape({
          ...
          rect: new Rect( { size: new Size(this.gridLineWidth, this.size.height),
      ...
      for (let row = 1; row < this.rows; row++) {
        const horizontalLine = new Shape({
          ...
          rect: new Rect( {size: new Size(this.size.width, this.gridLineWidth),
