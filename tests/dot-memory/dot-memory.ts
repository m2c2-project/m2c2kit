import { Game, Action, Timer, Transition, TransitionDirection, Scene, Shape, RectOptions, Size, Point, Label, LabelHorizontalAlignmentMode, Sprite, Entity, WebColors, Rect, RandomDraws, CompositeOptions, rgbaColor, EntityType, Composite, TextOptions } from "../../src/m2c2kit";
...
game.init({
  canvasId: 'mainCanvas', showFps: true,
  width: 360, height: 600,
  fontUrls: ["https://storage.googleapis.com/skia-cdn/google-web-fonts/Roboto-Regular.ttf"]
}).then(() => {
  ...
  const previousScreenTransition = Transition.push(TransitionDirection.right, 500);
  // going to next trial is shorter duration (250ms)
  const nextTrialScreenTransition = Transition.push(TransitionDirection.left, 250);
  ...
  const getReadyMessage = new Label({ text: 'Get Ready', position: new Point(180, 360) });
  gridMemoryPage0.addChild(getReadyMessage);

  gridMemoryPage0.setup(() => {
    gridMemoryPage0.run(Action.Sequence(Action.Wait(2000), Action.Code(() => {
  ...
  const page1Message = new Label({
    text: 'Remember the dot locations!', fontSize: 24,
    position: new Point(180, 60)
  });
  gridMemoryPage1.addChild(page1Message);

  const grid1 = new Grid({
    size: new Size(300, 300), position: new Point(180, 275), rows: 5, columns: 5,
  ...
  gridMemoryPage1.setup(() => {
    ...
    for (let i = 0; i < 3; i++) {
      // const circle = new Rectangle({ size: new Size(40,40), rx: 20, ry:20, fillColor: WebColors.Red,
      //   strokeColor: WebColors.Black, lineWidth: 2});
      const circle = new Shape({
        circleOfRadius: 20, fillColor: WebColors.Red,
        strokeColor: WebColors.Black, lineWidth: 2
      });
      grid1.addAtCell(circle, randomCells[i].row, randomCells[i].column);
    }

    gridMemoryPage1.run(Action.Sequence(Action.Wait(2000), Action.Code(() => {
  ...
  const touchTheFs = new Label({ text: "Touch the F's!", position: new Point(180, 30) });
  gridMemoryPage2.addChild(touchTheFs);

  const grid = new Grid({
    size: new Size(300, 480), position: new Point(180, 275), rows: 8, columns: 5,
    backgroundColor: WebColors.Transparent, gridLineColor: WebColors.Transparent
  });
  ...
  gridMemoryPage2.setup(() => {
    ...
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 5; j++) {
        const square = new Shape({ rect: new Rect({ size: new Size(59, 59) }), fillColor: WebColors.Transparent })
        square.userData = 0;
        ...
        if (letterIsF) {
          square.onTap(() => {
            if (square.userData === 0) {
              tappedFCount++;
              letter.text = 'E';
              letter.run(Action.Sequence(
                Action.Scale(1.25, 125), Action.Scale(1, 125)
              ));
              square.userData = 1;
              if (tappedFCount >= 6) {
                ...
                square.run(Action.Sequence(Action.Wait(1000), Action.Code(() => {
  ...
  const page3Message = new Label({
    text: 'Where were the dots?', fontSize: 24,
    position: new Point(180, 60)
  });
  ...
  gridMemoryPage3.setup(() => {
    ...
    for (let i = 0; i < 5; i++) {
      for (let j = 0; j < 5; j++) {
        const cell = new Shape({ rect: new Rect({ size: new Size(59, 59) }), fillColor: WebColors.Transparent })
        cell.userData = 0;
        cell.onTap(() => {
  ...
  const gridMemoryPage3DoneButton = new Shape({
    rect: new Rect({ size: new Size(300, 50) }), cornerRadius: 9,
    position: new Point(180, 500),
    fillColor: [44, 90, 255, 1]
  })
  gridMemoryPage3.addChild(gridMemoryPage3DoneButton);

  const gridDoneLabel = new Label({ text: 'Done', fontSize: 20, fontColor: [255, 255, 255, 1] })
  gridMemoryPage3DoneButton.addChild(gridDoneLabel);

  const youMustSelectAllMessage = new Label({ text: 'You must select all 3 locations!', position: new Point(180, 450) });
  ...
  gridMemoryPage3DoneButton.onTap(() => {
    if (tappedCellCount < 3) {
      youMustSelectAllMessage.run(Action.Sequence(
        Action.Code(() => {
