...
```js
Trial Data: {"session_uuid":"a274f6e1-79dc-4925-9468-29b906ac9acb",
...
- Lines 4 and 5 are the two variables that we defined in the trial schema: `trial_index` and `mobile_research_fun`.

- Lines 2 and 3 are uuids, or [universally unique identifiers](https://en.wikipedia.org/wiki/Universally_unique_identifier). For now, you can ignore these. Just know that these are automatically generated by m2c2kit to keep track of the activity (the assessment) and session (a set of activities). These are randomly generated and will be unique each time.

- Lines 6-22 are information about the device (metadata), automatically collected by m2c2kit. For now, you can ignore these. These are often useful for diagnostic purposes and to know how your users are interacting with your assessment.
...
{ description: <>Similarly, you will get an error if you try to save trial data for a variable that [does not exist] in the scema. This example incorrectly tries to add a value for a variable called `mobile_research_is_fun`, which does not exist: `game.addTrialData("mobile_research_is_fun", true)`</>,
code: `class DocsDemo extends Game {
    constructor() {
        const demoSchema = {
            trial_index: {
