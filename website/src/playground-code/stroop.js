// Click "Run", on the right side, to start this Stroop assessment.
//
// Tips:
  ...
  constructor() {
    const defaultParameters = {
      ...
      number_of_congruent_trials: {
        ...
        default: 4,
        description:
          "How many trials will be congruent (word matches its color).",
      },
      fixation_cross_duration_ms: {
        type: "number",
        default: 1000,
        description:
          "Duration, in milliseconds, of how long the fixation across will be shown.",
      },
      post_fixation_cross_blank_duration_ms: {
        type: "number",
        default: 1000,
        description:
          "Duration, in milliseconds, of how long a blank screen is shown after the fixation cross is hidden but before the word and response buttons are shown.",
      },
      show_feedback: {
        type: "boolean",
        default: true,
    ...
    const schema = {
      ...
      congruent_stimulus: {
        type: "boolean",
        description:
          "Did the word displayed on the screen match its font color?",
      },
      response_time_ms: {
        type: "number",
        description:
          "Time in milliseconds between when the word appears and the user taps a response button.",
      },
      response_correct: {
        type: "boolean",
        description:
          "Was the user's response correct? (Did the button response match the font color).",
      },
    };
    ...
    presentationScene.onAppear(() => {
      fixationCross.run(
        Action.sequence([
          ...
          Action.wait({
            duration: game.getParameter("fixation_cross_duration_ms"),
          }),
          Action.custom({
            callback: () => {
    ...
    endScene.onSetup(() => {
      if (game.getParameter("show_feedback")) {
        const percentCorrect =
          game.data.trials.filter((trial) => trial.response_correct).length /
          numberOfTrials;
        ...
        for (let i = 0; i < incongruentTrials.length; i++) {
          incongruentRtSum =
            incongruentRtSum + incongruentTrials[i].response_time_ms;
        }
        const meanIncongruentRt = incongruentRtSum / incongruentTrials.length;
        incongruentRtLabel.text =
          "Mean incongruent rt: " + meanIncongruentRt.toFixed(2);
      }
    });
  }
}
