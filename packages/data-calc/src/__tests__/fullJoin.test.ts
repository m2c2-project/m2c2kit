...
describe("fullJoin tests", () => {
  ...
  it("joins two datasets on a common variable", () => {
    ...
    expect(dc1.fullJoin(dc2, ["id"]).observations).toEqual([
      { id: 1, x: "a", y: 100 },
      { id: 2, x: "b", y: 200 },
      { id: 3, x: "c", y: null },
      { id: 4, x: null, y: 400 },
  ...
  it("keeps all rows from both datasets", () => {
    const dc1 = new DataCalc(d1);
    const dc2 = new DataCalc(d2);

    const result = dc1.fullJoin(dc2, ["id"]).observations;
  ...
  it("joins on multiple variables", () => {
    ...
    expect(dc1.fullJoin(dc2, ["id", "type"]).observations).toEqual([
  ...
  it("can join by complex object values", () => {
    ...
    expect(dc1.fullJoin(dc2, ["config"]).observations).toEqual([
  ...
  it("handles non-matching rows from both datasets", () => {
    const noMatches1 = [
      { id: 1, x: "a" },
      { id: 2, x: "b" },
    ];
  ...
  it("returns a new DataCalc object", () => {
    ...
    const result = dc1.fullJoin(dc2, ["id"]);
  ...
  it("handles empty datasets on both sides", () => {
    const dc1 = new DataCalc(d_empty);
    const dc2 = new DataCalc(d_empty);
  ...
  it("includes records with null join keys from both datasets but doesn't match them", () => {
    const nullKeys1 = [
      { id: 1, x: "a" },
      { id: null, x: "b" },
      { id: 3, x: "c" },
    ...
    // Full join should include all rows from both datasets
    // Records with id=null should be included but NOT matched with each other
    expect(dc1.fullJoin(dc2, ["id"]).observations).toEqual([
      { id: 1, x: "a", y: 100 },
      { id: null, x: "b", y: null },
      { id: 3, x: "c", y: null },
  ...
  it("includes records with undefined join keys from both datasets but doesn't match them", () => {
    ...
    // Full join should include all rows from both datasets
    // Records with undefined id should be included but NOT matched with each other
    expect(dc1.fullJoin(dc2, ["id"]).observations).toEqual([
      { id: 1, x: "a", y: 100 },
      { id: null, x: "b", y: null },
      { id: 3, x: "c", y: null },
